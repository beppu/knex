...
// knex instance.
var Builder = module.exports = function(builder) {
  this.builder    = builder;
  this.grammar    = builder.attributes.grammar;
  this.client     = builder.attributes.client;
  this.statements = [];
  this.bindings   = [];
  this.errors     = [];
};
...
Builder.prototype = {
  ...
  _compileCallback: function(callback, method) {
    ...
    builder.unchain();
    var builderInterface = new Builder(builder);
  ...
  _checkRaw: function(value, parameter) {
    if (value instanceof FluentChain) {
      var data = new Builder(value).toSql();
      this.binding.push(data.bindings);
      return '(' + data.sql + ')';
    }
...
for (var common in Common) {
  Builder.prototype[common] = Common[common];
}